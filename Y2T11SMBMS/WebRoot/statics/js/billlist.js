...
function deleteBill(obj){
	$.ajax({
		type:"GET",
		url:path+"/bill/"+obj.attr("billid")+"/delete",//不能为.html结尾，否则不能返回json(除非在控制器中显式指定返回格式)
		dataType:"json",
...
function openYesOrNoDLG(){
	...
	//$('#yes').attr('href','${pageContext.request.contextPath }/bill/'+obj.attr("billid")+'/delete.html');
	$('#removeBi').fadeIn();
}
...
$(function(){
	$(".viewBill").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/bill/"+obj.attr("billid")+"/view.html";
	});
	
	$(".modifyBill").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/bill/"+obj.attr("billid")+"/modify.html";
	});
	$('#no').click(function () {
		cancleBtn();
	});
